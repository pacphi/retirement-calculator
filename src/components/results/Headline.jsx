  ...
  return (
    <div className="rc-stat" style={{ background:"var(--header-bg)", borderRadius:14, padding:"22px 24px", color:"var(--header-ink)", marginBottom:16 }}>
      ...
      <div style={{ marginTop:14, display:"inline-flex", alignItems:"center", gap:8, background:onTrack?"color-mix(in srgb, var(--pos) 22%, transparent)":"color-mix(in srgb, var(--neg) 22%, transparent)", border:`1px solid ${onTrack?C.viridian:C.clay}`, borderRadius:999, padding:"6px 13px", fontSize:13, fontWeight:600 }}>
        <span style={{ width:8, height:8, borderRadius:99, background:onTrack?"var(--header-pos)":"var(--header-neg)" }} />
      ...
      <div style={{ marginTop:10, fontSize:11.5, color:"var(--header-mut)" }}>
...
function ShortfallWhy({ steady }) {
  ...
  const line = (label, value, opts = {}) => (
    <div style={{ display:"flex", justifyContent:"space-between", gap:12, fontSize:12.5, padding:"3px 0", color: opts.strong ? "var(--header-ink)" : "var(--header-mut)", fontWeight: opts.strong ? 700 : 400 }}>
      <span>{label}</span>
      <span style={{ fontFamily:"'JetBrains Mono',monospace", color: opts.neg ? "var(--header-neg)" : (opts.strong ? "var(--header-ink)" : "var(--header-mut)") }}>
        {opts.neg ? "-" : ""}{usd0(Math.abs(value))}{opts.suffix || ""}
      </span>
    </div>
  );
  return (
    <div style={{ marginTop:12, padding:"12px 14px", background:"color-mix(in srgb, var(--neg) 12%, transparent)", border:"1px solid color-mix(in srgb, var(--neg) 35%, transparent)", borderRadius:11 }}>
      <div style={{ fontSize:11, letterSpacing:1, textTransform:"uppercase", color:"var(--header-neg)", fontWeight:700, marginBottom:6 }}>
        Why it falls short
      </div>
