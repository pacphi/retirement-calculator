...
// Shared by every pensionType's result summary below -- was hand-copied 9 times.
const RESULT_BOX_STYLE = { fontSize:12, color:C.slate, background:"var(--surface-2)", borderRadius:8, padding:"9px 11px", lineHeight:1.5 };
function PensionResultBox({ children }) {
  return <div style={RESULT_BOX_STYLE}>{children}</div>;
}

// Shared AFC/High-3/final-compensation field, used by every pensionType except "generic"
// (which has no formula-derived compensation input). Also the "reset to assumed" affordance --
// previously present only in the drs/fers blocks and missing from the other 6 pensionTypes.
function CompensationField({ label, hint, afcAuto, afcEff, incomeB, set }) {
  return (
    <Field
      label={<>{label}{afcAuto && <AssumptionIcon title={`Auto-filled from the spouse's current income because no ${label.toLowerCase()} was entered. Wages are held flat in real terms, so today's salary stands in for it. Type a value to override.`} />}</>}
      hint={hint}
    >
      <NumberInput value={afcEff} onChange={set("afc")} prefix="$" suffix="/yr" />
      {afcAuto
        ? <span style={{ display:"block", fontSize:11, color:C.brassDeep, marginTop:5, lineHeight:1.4 }}>Assumed from the spouse's income ({usd0(Number(incomeB)||0)}) — wages are held flat in real terms, so this is a placeholder. Type a value to override.</span>
    ...
    <Section eyebrow="Step five" title="Spouse's pension">
      ...
      {s.pensionOn && (<>
        ...
        {pensionType === "drs" && (<>
          ...
          <CompensationField label="Average final compensation (AFC)" hint="Avg pay over the spouse's highest 60 consecutive months." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
        ...
        {pensionType === "fers" && (<>
          ...
          <CompensationField label="High-3 average salary" hint="Average of the highest 3 consecutive years of basic pay." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
        ...
        {pensionType === "calstrs" && (<>
          ...
          <CompensationField label="Final compensation" hint="Highest 12 or 36 consecutive months, per the tier's rule." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
        ...
        {pensionType === "calpers" && (<>
          ...
          <CompensationField label="Final compensation" hint="Highest 12 (Classic) or 36-month average (PEPRA), per employer contract." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
        ...
        {pensionType === "txtrs" && (<>
          ...
          <CompensationField label="Average of 5 highest annual salaries" hint="Grandfathered members (vested by Aug 31, 2005) use a 3-year average instead — not modeled separately here." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
        ...
        {pensionType === "nystrs" && (<>
          ...
          <CompensationField label="Final average salary (3-yr)" hint="Highest 3 consecutive years." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
        ...
        {pensionType === "ohiostrs" && (<>
          ...
          <CompensationField label="Final average salary (5-yr)" hint="Average of the 5 highest years of Ohio earnings." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
        ...
        {pensionType === "military" && (<>
          ...
          <CompensationField label="High-3 average basic pay" hint="Average of the highest 3 years of basic pay." afcAuto={afcAuto} afcEff={afcEff} incomeB={s.incomeB} set={set} />
          <PensionResultBox>
