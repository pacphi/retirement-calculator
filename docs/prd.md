...
| Glidepath (Wave 3) | Equity allocation de-risks linearly during the accumulation phase only: `equityPctNow` (default 80%) at plan start down to `equityPctAtRetire` (default 40%) at the retirement year, via `progress = (totalAccumYears − yearsToRetire) / totalAccumYears`; post-retirement holds `equityPctAtRetire` (no age-85 endpoint). Blended real return = equity% × `equityReal` + bond% × `bondReal`. Monte Carlo samples the single blended (midpoint) mean, so band width is unchanged. Sources: `SOURCES.cfa6040`, `SOURCES.carson6040`. |
